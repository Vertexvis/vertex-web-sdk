...
| `enabledSelectionModifierKeys` | --                            | A set of supported modifier keys when performing selection. Setting any value to `false` will cause a click with that modifier key to be ignored and no selection to be performed. | `SelectionModifierKeys`                                | `{     shiftKey: true,     ctrlKey: true,     metaKey: true,     altKey: true,   }` |
