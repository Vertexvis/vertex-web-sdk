...
| `viewMatrix`            | --                        | The view matrix that specifies the camera's orientation. If `viewer` is set, this property will be populated automatically.             | `[number, number, number, number, number, number, number, number, number, number, number, number, number, number, number, number] \| undefined` | `Matrix4.makeIdentity()` |
